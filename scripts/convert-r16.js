// Merges Round-of-16 picks into public/picks.json from "Round of 16.xlsx".
//
...
// into each participant under that id. It MERGES (group + R32 picks are
...
// R32 winners) to the bracket pairings in src/app/data/knockout-schedule.ts.
// Team names are canonicalized to the forms the rest of the app uses (flags.ts).
//
// Run: npm run convert:r16   (run AFTER npm run convert, which overwrites picks.json)
...
const XLSX_PATH = path.join(ROOT, '..', 'Round of 16.xlsx');
...
// Each R16 matchup -> its FIFA match id, keyed by the canonical home team (the
...
// (each game's "Wxx vs Wyy" pairing + the R32 winners).
...
function main() {
  ...
  if (unmatched.length) {
    console.error(`WARNING: ${unmatched.length} R16 name(s) not found in picks.json: ${unmatched.join(', ')}`);
  }
