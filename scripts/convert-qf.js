// Merges Quarterfinal picks into public/picks.json from "Quarter-Finals.xlsx".
//
// After the Round of 16 a follow-up Google Form collected each participant's
// Quarterfinal winner picks. That export ("Quarter-Finals.xlsx") has one sheet:
//   Name | <4 QF matchup columns "Home vs. Away">
...
// knockout games use their FIFA match numbers as ids (97..100 for the QF), so
// this script maps each matchup column to its FIFA match id and writes the pick
// into each participant under that id. It MERGES (group + R32 + R16 picks are
// preserved) and is idempotent, so it can be re-run safely.
//
// The matchup -> FIFA id mapping was fixed by matching each matchup's teams (the
// R16 winners) to the bracket pairings in src/app/data/knockout-schedule.ts.
// Team names are canonicalized to the forms the rest of the app uses (flags.ts).
//
// Run: npm run convert:qf   (run AFTER npm run convert, which overwrites picks.json)
...
const XLSX_PATH = path.join(ROOT, '..', 'Quarter-Finals.xlsx');
...
// Each QF matchup -> its FIFA match id, keyed by the canonical home team (the
...
// (each game's "Wxx vs Wyy" pairing + the R16 winners).
...
function main() {
  ...
  if (unmatched.length) {
    console.error(`WARNING: ${unmatched.length} QF name(s) not found in picks.json: ${unmatched.join(', ')}`);
  }
  ...
  console.log(`  QF matchups (id: home vs. away):`);
  matchups
    .sort((a, b) => a.id - b.id)
